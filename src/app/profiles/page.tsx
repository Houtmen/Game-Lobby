...
import type { GameProfile } from '@/types/phase4c';
...
export default function ProfilesPage() {
  ...
  const remove = async (id: string) => {
    if (!token) return;
    await fetch(`/api/profiles?id=${encodeURIComponent(id)}`, {
      method: 'DELETE',
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300 mb-6">Please log in to manage profiles.</p>
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-blue-400">Profiles</h1>
        </div>
        ...
        <ul className="space-y-3">
          {profiles.map((p) => (
            <li key={p.id} className="bg-gray-800 border border-gray-600 rounded p-3 flex items-center justify-between">
              <div>
                <div className="font-medium flex items-center gap-2 text-white">
                  {p.name}
                  {p.isDefault && <span className="text-xs text-green-200 bg-green-700/40 border border-green-600 px-1 rounded">default</span>}
                </div>
                <div className="text-xs text-gray-400">gameId: {p.gameId}</div>
              </div>
              <div className="flex gap-2 items-center">
                {!p.isDefault && (
                  <Button
                    ...
                    onClick={async () => {
                      if (!token) return;
                      await fetch(`/api/profiles?id=${encodeURIComponent(p.id)}&action=setDefault`, {
                        method: 'PATCH',
                        headers: { Authorization: `Bearer ${token}` },
                      });
                      await load();
                ...
                <Button variant="rose" onClick={() => remove(p.id)}>Delete</Button>
              </div>
            </li>
          ))}
        </ul>
