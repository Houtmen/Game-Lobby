...
export async function GET(
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ session: formattedSession }),
      { 
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      }
...
export const PUT = requireAuth(async (
  ...
  try {
    ...
    if (session.hostId !== request.user!.userId) {
      return new Response(
        JSON.stringify({ error: 'Only the host can update this session' }),
        { 
          status: 403,
          headers: { 'Content-Type': 'application/json' }
        }
...
export async function DELETE(
  ...
  try {
    ...
    if (session.hostId !== userPayload.userId) {
      return new Response(
        JSON.stringify({ error: 'Only the host can delete this session' }),
        { 
          status: 403,
          headers: { 'Content-Type': 'application/json' }
        }
    ...
    return new Response(
      JSON.stringify({ message: 'Session cancelled successfully' }),
      { 
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      }
