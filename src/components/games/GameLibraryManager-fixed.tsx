    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        {error && (
          ...
          >
            <p>{error}</p>
            <Button onClick={() => setError(null)} variant={error.startsWith('✅') ? 'green' : 'rose'} padding="sm">Dismiss</Button>
          </div>
        )}
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-8 border border-gray-600">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <Button onClick={() => scanForGames(false)} disabled={scanning || loading} variant="purple">{scanning ? 'Scanning System...' : 'Scan Common Locations'}</Button>
            <Button onClick={() => scanForGames(true)} disabled={scanning || loading || customPaths.length === 0} variant="indigo">{scanning ? 'Scanning...' : 'Scan Custom Paths'}</Button>
            <Button onClick={() => setShowManualAdd(true)} disabled={scanning} variant="blue">Add Game Manually</Button>
            <Button onClick={loadGames} disabled={loading} variant="green">{loading ? 'Loading...' : 'Refresh List'}</Button>
          </div>
