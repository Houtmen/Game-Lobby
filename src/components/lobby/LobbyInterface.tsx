...
import { FaGamepad, FaUsers, FaCog, FaPaperPlane, FaMicrophone, FaSignOutAlt, FaPlay, FaPause, FaCrown, FaCheck, FaTimes } from 'react-icons/fa'
...
export const LobbyInterface: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initSocket = async () => {
      ...
      socketInstance.on('player_joined', (player: { userId: string; username: string; avatar?: string }) => {
        setLobbyState(prev => {
          if (!prev.currentRoom) return prev
          const updatedRoom = {
            ...prev.currentRoom,
            players: [...prev.currentRoom.players, {
              ...player,
              isHost: false,
              isReady: false,
              joinedAt: new Date()
  ...
  const currentPlayer = lobbyState.currentRoom?.players.find(p => p.userId === user?.id)
  const allPlayersReady = lobbyState.currentRoom?.players.every(p => p.isReady) && 
      ...
      {!lobbyState.currentRoom ? (
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300">Welcome, {user.username}! Join a game session or create one.</p>
          </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                ...
                {lobbyState.loadingSessions ? (
                      ...
                      <div key={session.id} className="bg-gray-700 rounded-lg p-4">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              Host: {session.host.username} • {session.players.length}/{session.maxPlayers} players
                            </p>
                          </div>
                          <button
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${
                            session.status === 'WAITING' ? 'bg-green-600' : 
                            session.status === 'STARTING' ? 'bg-yellow-600' : 'bg-gray-600'
                          }`}>
                            {session.status}
                          </span>
                        </div>
                  ...
                  <div className="bg-gray-700 rounded-lg p-4 text-center">
                    ...
                    <p className="text-sm text-gray-500">Create a new game session to get started</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="mb-6">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-blue-400">{lobbyState.currentRoom.gameName}</h1>
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {lobbyState.currentRoom.players.map((player) => (
                  <div key={player.userId} className="flex items-center justify-between bg-gray-700 rounded p-3">
                    <div className="flex items-center">
